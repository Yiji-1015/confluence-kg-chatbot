...
현재 구현 범위: 날짜가 포함된 제목을 여러 표현으로 확장하는 색인은 적용하지 않는다. 과거 `title_search`와 `expand_title_dates()`는 제거됐다. 현재는 원본 제목을 보존하고 임베딩 입력에 제목과 청크 본문을 함께 넣는다. 날짜 확장 실험은 [변경 기록](../CHANGELOG.md)의 2026-09-13 절에만 과거 이력으로 해석한다.
