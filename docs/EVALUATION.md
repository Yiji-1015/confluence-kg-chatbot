...
실측(2026-09-16, 5.3절): `boundary` 축은 `hit` 0.800인데 `ragas_context_precision`은 0.362다.
...
첨부파일명이 프롬프트에 들어가지 않아 답할 근거 자체가 없었다(5.5절).
...
뒤의 두 유형(5문항)은 사실 서술이 아니라 **동작 서술**이다. 그래서 전체 평균 0.800은
...
| 항목 | 값 |
|---|---|
| Langfuse run | `ey-interview-36-rrf-k60-top5-20260916` |
...
| 실행 결과 | 36 item / Error 0 / **채점 누락 0** |

## 5.2 여섯 지표

| 지표 | 값 | 채점 문항 |
|---|---|---|
| `retrieval_hit` | **0.818** | 33 / 33 |
| `retrieval_mrr` | **0.553** | 33 / 33 |
| `answer_faithfulness` (자체) | **0.992** | 36 / 36 |
| `answer_correctness` | **0.800** | 36 / 36 |
| `ragas_faithfulness` (표준) | **0.879** | 36 / 36 |
| `ragas_context_precision` | **0.679** | 36 / 36 |

검색 지표의 분모가 33인 것은 정답 `page_id`가 없는 `not_found` 3문항을 뺐기 때문이다(2.1절).

**검색 지표가 별도 측정과 일치한다.** `retrieval_hit` 0.818 / `retrieval_mrr` 0.553은
...
|---|--:|--:|--:|--:|--:|--:|--:|
| semantic | 8 | 0.750 | 0.490 | 0.981 | 0.688 | 0.899 | 0.533 |
| keyword | 7 | 0.857 | 0.607 | 0.979 | 0.986 | 0.892 | 0.750 |
| boundary | 7 | 0.800 | 0.357 | 1.000 | 0.971 | 0.882 | 0.362 |
| conversational | 6 | 0.833 | 0.492 | 1.000 | 0.900 | 0.957 | 0.881 |
| attachment | 6 | 0.800 | 0.667 | 1.000 | **0.367** | 0.721 | 0.853 |
| table | 2 | 1.000 | 1.000 | 1.000 | 1.000 | 0.974 | 1.000 |

축별 문항 수가 2~8이다. **방향 지표로만 쓰고 절대값을 인용하지 않는다.**

`answerability`별 `answer_correctness`: answerable 0.782(28) / answerable_negative 0.967(3) /
not_found **1.000**(3) / unsupported 0.500(2).
**없는 문서를 묻는 3문항은 전부 정직하게 거절했다.**

## 5.4 실패 문항 (correctness < 0.7, 8건)

| id | category | hit | mrr | faith | corr | 진단 |
|--:|---|--:|--:|--:|--:|---|
| 8 | semantic | 0.0 | 0.00 | 1.0 | 0.0 | 검색 실패 — 정답이 **6위**, top5 바로 밖 |
| 10 | semantic | 1.0 | 0.33 | 1.0 | 0.0 | 문서는 찾았으나 이해·추론 실패 |
| 11 | semantic | 0.0 | 0.00 | 0.9 | 0.6 | 검색 실패 — 후보 풀 밖 (아래) |
| 16 | conversational | 1.0 | 0.50 | 1.0 | 0.5 | 부분 정답 |
| 25 | attachment | 1.0 | 1.00 | 1.0 | 0.2 | 첨부 축 (5.5절) |
| 33 | attachment | 0.0 | 0.00 | 1.0 | 0.0 | **최신성 가산점 때문** (아래) |
| 34 | attachment | 1.0 | 0.33 | 1.0 | 0.0 | 첨부 축 (5.5절) |
| 35 | attachment | 1.0 | 1.00 | 1.0 | 0.0 | 첨부 축 (5.5절) |

`answer_faithfulness`는 8건 중 7건이 1.0이다. **컨텍스트를 무시하거나 지어내서 틀린 것이
아니라, 컨텍스트에 답이 없는데 그 안에서 성실하게 답한 결과다.** 자체 faithfulness만 보면
0.992로 문제가 없어 보이지만 correctness는 0.800이다. 두 지표를 함께 봐야 하는 이유다.
...
## 5.5 확인된 원인 — 첨부파일명이 컨텍스트에 들어가지 않는다

`attachment` 축만 correctness 0.367로 무너지는데 `hit`은 0.800으로 정상이다.
**검색은 되는데 답변이 안 된다.** 원인을 코드와 실제 컨텍스트로 확인했다.

- 검색은 첨부명을 본다. BM25 필드에 `attachments^1.0`이 있어 첨부파일명으로 문서를 찾아낸다.
- **생성은 첨부명을 못 본다.** `build_context_text()`가 렌더링하는 것은 `title` / `path` /
  `text` 셋뿐이고, `search_hybrid`가 함께 돌려주는 `attachments`를 프롬프트에 넣지 않는다.
...
| 컨텍스트에 그 파일명이 있는가 | **없음** |
| 실제 답변 | "파일 수령 가능 여부는 문서에 명시되어 있지 않습니다" |

#25는 correctness 0.2지만 첨부명을 답했다. 그 문서는 **본문에도** 첨부명이 적혀 있어
`text`를 통해 우연히 컨텍스트에 들어갔기 때문이다. 즉 지금 첨부 질문에 답할 수 있는지는
"본문에 파일명이 적혀 있었는가"라는 운에 달려 있다.

#35(`unsupported`)는 반대 방향의 실패다. 첨부 본문이 색인되지 않아 **답하지 말아야 하는데**
같은 페이지의 다른 본문을 근거로 연차 소멸 기한을 상세히 답했다.

**이 절은 원인 기록이고 수정은 하지 않았다.** 고치려면 `build_context_text()`가 `attachments`를
렌더링해야 하며, 그 변경은 6절의 범위다.
...
- **36문항 1회 측정이다.** ±0.03 수준의 차이는 노이즈와 구분되지 않는다.
- 축별 문항이 2~8건이라 **category별 절대값은 인용할 수 없다.** 방향만 본다.
- 정답 문서가 18종에 몰려 있다. 문서 단위 편향이 지표에 그대로 남는다.
- `answer_correctness` 0.800은 사실 정확도와 응답 정책 준수를 섞은 값이다(4.2절).
...
| 우선순위 | 할 일 | 근거 |
|---|---|---|
| 1 | `build_context_text()`가 `attachments`를 렌더링 | 5.5절. attachment 축 correctness 0.367 |
| 2 | `RECENCY_BOOST_MAX` 유지 여부 결정 | 5.4절. 끄면 #33을 되찾지만 근거가 1문항뿐 |
| 3 | #11의 1차 후보 회수 개선 (후보 50 밖) | 5.4절. 결합 방식으로는 못 고친다 |
| 4 | #8의 대형 문서 청크 희석 (29청크) | 5.4절. 청킹 전략 과제 |
