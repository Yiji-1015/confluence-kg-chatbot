...
## 5.2 생성 지표 (2026-09-14 / 45문항 / 판정 `solar`)

실행 이름: `qa-rrf_k60-top5-cand50-chars3000-recency0.04-temp0-judge_solar-judge-0913-1942`

| 지표 | 값 | 채점 문항 |
|---|---|---|
| `answer_correctness` | **0.958** | 45 / 45 |
| `answer_faithfulness` (자체) | **0.973** | 45 / 45 |
| `ragas_faithfulness` (표준) | **0.856** | 45 / 45 |
| `ragas_context_precision` | **0.851** | 45 / 45 |

**여섯 지표 모두 45문항이 온전히 채점된 첫 실행이다** (`채점 누락 없음`).
RRF 전환(09-14)과 RAGAS `max_tokens` 수정(09-14) 이후 측정이다.

이전 측정과의 비교 (조건이 다르므로 참고용):

| 지표 | 2026-09-08 (min-max) | 2026-09-14 (RRF) |
|---|---|---|
| `answer_correctness` | 0.970 | 0.958 |
| `ragas_context_precision` | 0.753 | **0.851** |

`context_precision`이 0.753 → 0.851로 올랐다. 가져온 문서 중 실제로 답변에 쓰인 비율이며,
검색 지표(`hit`/`MRR`)로는 드러나지 않는 개선을 이 지표가 잡아냈다.
`correctness` 차이 0.012는 45문항 중 한 문항이 안 되는 폭이라 노이즈 범위다.

**남은 실패 1건** — `qa-v2-021`("LLOYDK 팀의 3월 3일 주간미팅"),
`hit=0.0 / faithfulness=1.0 / correctness=0.0` → 검색 실패로 자동 분류됐다.
6절의 "남은 검색 실패 1건"에 원인이 정리돼 있다.

## 5.3 자체 지표와 표준 지표의 차이

| 측정 시점 | `answer_faithfulness` (자체) | `ragas_faithfulness` (표준) | 차이 |
|---|---|---|---|
| 2026-09-01 기준선 | 0.951 | 0.838 | **0.113** |
| 2026-09-14 (RRF, 누락 0) | 0.973 | 0.856 | **0.117** |

결합 방식과 분석기를 여러 번 바꾸는 동안에도 **격차가 0.11~0.12로 일정하다.**
우연이 아니라 두 구현의 판정 방식 차이에서 오는 구조적 편향이라는 뜻이다.
...
| 실행 | 누락 | 원인 |
|---|---|---|
| 2026-09-02 | 5문항 | 판정 모델 429. 재시도가 없어 조용히 빠졌다 → 게이트웨이에 `num_retries: 3` |
| 2026-09-08 ~ 09-13 | 1~5문항 | `ragas_faithfulness`의 `IncompleteOutputException` → `max_tokens` 4096 |
| **2026-09-14** | **0문항** | — |

두 번 모두 **실행은 정상 종료되고 점수도 그럴듯했다.** 누락 경고 장치가 없었다면
40문항 평균을 45문항 평균으로 알고 넘어갔을 것이다.
