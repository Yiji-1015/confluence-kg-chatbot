...
| best_fields + RRF(k=60) | 0.974 | 0.934 |

필드 결합(축 1)을 바꿔도 거의 변화가 없고, 검색기 결합(축 2)을 RRF로 바꾼 것만 올랐다.
다만 38문항에서 2건 차이라 표본이 작고, RRF는 상위 5개 구성이 33건에서 달라지는데
그 답변 품질 영향은 미측정이다. **근거가 확정되기 전까지 현재 방식을 유지한다.**
...
- **min-max 정규화의 절벽**: 각 리스트의 꼴찌가 항상 정확히 0.0이라, 후보 풀 마지막
  ...
  결합은 `_normalize_scores`(chunk_id -> 0~1) 하나에 격리돼 있어 교체 자체는 몇 줄이다.

  **주의 - RRF로 바꾸면 `RECENCY_BOOST_MAX`를 반드시 다시 잡아야 한다.** 두 방식의
  점수 스케일이 두 자릿수 차이라, 지금 값을 그대로 두면 가산점이 지배적 요소가 된다.

  | 방식 | 결합 점수 범위 | `RECENCY_BOOST_MAX=0.04`의 비중 |
  |---|---|---|
  | min-max (현재) | 0 ~ 10 | 0.4% (동점 근처에서만 작동, 설계 의도대로) |
  | RRF(k=60) | 0 ~ 0.16 | 25% (사실상 최신순 정렬) |

  검색 지표만 보면 이 붕괴가 드러나지 않을 수 있다. 최신 문서가 정답인 문항에서는
  오히려 점수가 오르기 때문이다. 전환한다면 recency를 끈 상태로 먼저 비교해야 한다.
