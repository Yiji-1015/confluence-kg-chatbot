...
효과가 확인되지 않아 기본값은 0.0(끔)으로 두되, 코드와 측정 기록은 남긴다.
`updated_at`은 Confluence 최종 수정 시각이라 "문서 내용의 날짜"와 다르다는 점도
최신성 신호로 쓸 때의 제약이다.
...
- `qa-v2-007`에서 `answer_correctness`가 `None`으로 빠졌다. 판정 응답이
  `SCORE:` 형식을 벗어나면 정규식 파싱이 실패하고 그 문항이 **조용히 채점에서 제외**된다.
  실행은 정상 종료되고 평균 점수도 멀쩡해 보이므로 발견이 어렵다.
  파싱 실패 건수를 집계해 출력하는 것이 필요하다.
