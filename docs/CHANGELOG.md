...
추가분 135건은 **고유 132종 전부가 사람이 지은 이름이었고 자동 생성이 0종**이라 1차
(pdf/excel)보다도 깨끗했다. 제안서·매뉴얼·회의록·주간업무보고·설계서가 대부분이다.

| | 문서 | 고유 첨부명 | 첨부 전용 문서 |
|---|---|---|---|
| pdf/excel만 | 102 | 234종 | 29 |
| 문서 형식 전반 | **126** | **364종** | **34** |

이름에 날짜가 붙은 것들(`[LGU+] 사용자 매뉴얼-ver1.1(20250310)`, `K-water_주간업무보고_260828`)은
그대로 둔다. 자동 생성 이름과 달리 사람이 지은 이름의 일부이고, 숫자를 이유로 버리면
진짜 문서명을 잃는다.

**가중치를 1.5에서 1.0으로 내렸다.** 색인 대상을 넓히자(고유 234종 -> 364종) 1.5가
v4 paraphrase에서 손해로 돌아섰다. 첨부 데이터가 늘수록 높은 가중치의 간섭도 커진다.

| 가중치 | v2 hit@5 / MRR | v4 hit@5 / MRR |
|---|---|---|
| `attachments^1.5` | 0.974 / 0.895 | **0.684** / 0.520 |
| `attachments^1.0` | 0.974 / 0.890 | **0.711** / 0.525 |
| 첨부 제외 | 0.974 / 0.890 | 0.711 / 0.525 |

차이는 38문항 중 1문항이라 노이즈 범위다. 다만 1.0이 어느 쪽에서도 손해가 아니면서
첨부로만 찾히는 문서는 그대로 찾으므로 1.0을 택했다. 가중치 1.0에서도 제목·본문에 없고
첨부명에만 있는 단어로 질의했을 때 3건 중 2건이 이 필드로만 찾아지는 것을 확인했다.
첨부 질문이 포함된 평가셋이 생기면 그때 다시 정한다.

새로 색인된 첨부 전용 문서 34건이 정답을 top5에서 밀어낸 경우는 0건이었다.

### 첨부파일 이름이 색인에 한 번도 들어간 적이 없었다

"첨부파일 이름 파싱해온 게 ingest가 안 된다"는 제보로 시작했는데, 끊긴 지점이 다섯 군데였다.
`git log -S attachments` 기준 초기 커밋 이후 손댄 적이 없다. 회귀가 아니라 처음부터
연결되지 않은 코드였다.

| # | 문제 | 위치 |
|---|---|---|
| 1 | 파서 결과의 `attachments`를 읽지 않고 버림 | `scripts/ingest.py` |
| 2 | ES 매핑과 BM25 검색 필드에 `attachments` 없음 | `retrieval/es_client.py` |
| 3 | 매크로로 삽입된 첨부가 추출 전에 삭제됨 | `parser/confluence_parser.py` |
| 4 | 본문 없는 첨부 전용 페이지가 청크 0개로 유실 | `parser/confluence_parser.py` |
| 5 | 검색 결과 조립에서 `attachments` 누락 | `retrieval/es_client.py` |

**3번이 가장 컸다.** Confluence의 `view-file`/`viewpdf` 매크로는 첨부 참조를
`<ac:parameter ac:name="name"><ri:attachment .../></ac:parameter>`에 넣는데, `ac:parameter`는
`_NOISE_TAGS`라 `decompose()` 대상이다. 그 제거가 추출보다 먼저 돌고 있었다. pdf/excel은
대부분 이 매크로로 삽입되므로 손실이 특히 컸다 (고유 234종 중 73종만 남았다).
추출을 노이즈 제거 앞으로 옮겼다. 첨부 보유 문서 20건 -> 102건.

**4번은 가치가 가장 높은 쪽이었다.** pdf/excel 첨부를 가진 102문서 중 29문서가 본문이
비어 있었다. `split_text_into_chunks`가 빈 본문에 청크를 0개 만들어 색인에 아예 남지
않았다. 그런데 그게 하필 `취업규칙`, `사업자등록증`, 각종 인증서처럼 **제목이 주제이고
첨부 PDF가 내용 전부인 페이지**였다. "취업규칙 어디 있어"에 아무것도 걸리지 않았다.
첨부 이름을 본문 삼아(`첨부파일: 20260325_취업규칙_V3`) 청크 하나를 만들도록 했다.

### 첨부파일은 pdf/excel만 색인한다

확장자 허용목록(`_INDEXED_ATTACHMENT_EXTS`)으로 거른다. 이름 휴리스틱도 만들어봤으나
`20240501_강소기업확인서` 같은 진짜 문서명을 숫자 때문에 버려서 폐기했다.

실측 (670문서 / 첨부 참조 925건):

| 확장자 | 건수 | 비율 | 처리 |
|---|---|---|---|
| `.png` `.jpg` `.jpeg` `.gif` | 483 | 52.2% | 제외 |
| `.pdf` | 200 | 21.6% | 색인 |
| `.xlsx` | 50 | 5.4% | 색인 |
| `.pptx` `.docx` `.hwp` `.hwpx` | 135 | 14.6% | 제외 (넓힐 후보) |

이미지 이름은 거의 전부 자동 생성이라 색인해도 쓸모없을 뿐 아니라 **해롭다**.
`image-20260914-001446`이 `image` / `20260914` / `001446`으로 쪼개지는데, 코퍼스에 몇 건
없어 IDF가 극단적으로 높다. 같은 함정을 이미 겪었다 (날짜 토큰 `20260303`이 3,211청크 중
12건뿐이라 BM25를 지배한 건, 2026-09-13 항목). 스크린샷 붙여넣은 날짜로 엉뚱한 문서가
상위에 오는 일이 생긴다.

거른 뒤 남는 251건(고유 234종)은 전부 사람이 지은 문서명이다.

### 검증

색인된 고유 첨부명 0종 -> **234종** (102문서 / 260청크). 조사 예상치와 정확히 일치해 유실 0.

**첨부 필드의 기여를 분리 확인했다.** 제목·본문에 없고 첨부명에만 있는 단어로 질의하니
3건 중 2건이 `attachments` 필드가 있어야만 찾아졌다. 없으면 완전히 못 찾는다.

**기존 검색을 깨지 않았다.** 같은 인덱스에서 필드만 넣고 뺀 A/B:

| 필드 구성 | v2 hit@5 / MRR | v4 hit@5 / MRR |
|---|---|---|
| `attachments^1.5` 포함 | 0.974 / 0.895 | 0.711 / 0.526 |
| 첨부 제외 | 0.974 / 0.890 | 0.711 / 0.525 |

새로 색인된 첨부 전용 문서 29건이 정답을 top5에서 밀어낸 경우도 0건이었다.

**주의: 확정 지표가 현재 인덱스와 맞지 않는다.** 2026-09-15에 고정한 값은 583문서·3,211청크
기준인데 지금은 614문서·3,488청크다. Confluence가 그 사이 늘어난 탓이고 이번 변경과는
무관하다(위 A/B로 분리 확인). v4 `hit@5` 0.763 -> 0.711도 이 코퍼스 변화 때문이다.
다시 측정할 때 `EVALUATION.md` 5절의 측정 조건도 같이 갱신해야 한다.

**`attachments^1.5`는 확정값이 아니다.** 가중치 1.0과 0.5가 v4에서 오히려 나았으나
(`hit@5` 0.737 vs 0.711) 38문항 중 1문항 차이라 노이즈 범위다. 한 문항으로 정하지 않는
기준을 따라 1.5로 뒀다. `_BM25_FIELDS` 상수로 빼뒀으니 평가셋을 새로 만든 뒤 정한다.

## 2026-09-15

### 지표 확정 — 09-14 실행을 기준값으로 고정하고 문서를 맞춤

재측정 없이 문서상 최종값을 고정했다. 기준은 `EVALUATION.md` 5절의 09-14 실행
(`qa-rrf_k60-top5-cand50-chars3000-recency0.04-temp0-judge_solar-judge-0913-1942`),
여섯 지표가 45문항 전부 채점된 유일한 실행이다.

**`PORTFOLIO.md`의 측정값이 어느 실행과도 맞지 않았다.**

| 지표 | PORTFOLIO.md (09-01로 표기) | CHANGELOG 09-01 정식 기록 | 확정값 (09-14) |
|---|---|---|---|
| `answer_faithfulness` (자체) | 0.989 | 0.951 | **0.973** |
| `ragas_faithfulness` (표준) | 0.861 | 0.838 | **0.856** |
| `answer_correctness` | 0.966 | 0.970 | **0.958** |
| `ragas_context_precision` | 0.800 | 0.835 (recency=0) | **0.851** |

`EVALUATION.md` 5.3절과 `PORTFOLIO.md`가 같은 09-01 기준선을 서로 다르게 적고 있었다.
기준선 측정 기록(아래 "기준선 측정 결과 (2026-09-01)")과 대조한 결과 `EVALUATION.md` 쪽이
맞고, `PORTFOLIO.md`의 네 값은 어느 실행에도 대응하지 않는 옛 초안 값이었다.
`0.800`만은 recency를 켠 09-02 실행의 값인데, 그 실행은 429로 5문항이 빠진 40문항 평균이다.
**출처를 특정할 수 없는 값이 포트폴리오 문서에만 남아 있었다는 뜻이다.**

측정값과 함께 측정 조건도 어긋나 있어 같이 맞췄다.

- "해결"과 "주요 구현"이 `BM25 40% : kNN 60% 정규화 가중합`으로 남아 있었다. 확정 지표는
  RRF(K=60) 기준 측정값이라 본문이 측정 조건과 모순이었다.
- "알려진 한계"의 코퍼스 규모 `2,979청크` -> `3,211청크`. 09-13 재색인 이후 값이다.
- 최신 문서 가산점의 `context_precision -0.035`에 "09-02, min-max 시절 측정" 단서를 붙였다.
  현재 결합 방식에서 다시 잰 값이 아니다.
- `PORTFOLIO_SLIDES.md`의 "최근 커밋 09-13" -> `09-14`.

코드와 평가 파이프라인은 건드리지 않았다. 문서만 기준값에 맞춘 변경이다.

**Langfuse 원본 점수와 대조했다 (2026-09-15).** 문서 값이 실제 실행 점수와 같은지
Public API로 확인했다. 실행 `059453da-70a5-43f6-bd7d-b11f8660fef0`의 45개 run item에
달린 score 레코드를 전부 받아 지표별로 평균했다.

| 지표 | Langfuse 실측 | 채점 | 문서 확정값 |
|---|---|---|---|
| `retrieval_hit` | 0.9737 | 38 / 38 | 0.974 |
| `retrieval_mrr` | 0.9079 | 38 / 38 | 0.908 |
| `answer_correctness` | 0.9578 | 45 / 45 | 0.958 |
| `answer_faithfulness` | 0.9733 | 45 / 45 | 0.973 |
| `ragas_faithfulness` | 0.8565 | 45 / 45 | 0.856 |
| `ragas_context_precision` | 0.8515 | 45 / 45 | 0.851 |

소수점 셋째 자리까지 전부 일치한다. **채점 누락 0건도 원본에서 확인됐다.**
`hit=0`인 문항은 `qa-v2-021` 하나뿐이고 `faithfulness=1.0 / correctness=0.0`으로,
문서에 적힌 "남은 실패 1건"과 같다.

검색 지표가 38문항인 이유도 원본에서 확인된다. 정답 문서 라벨이 없는 7건
(`gap-01~02`, `ood-01·02·03·05·06`)이 빠진다. `ood-04`는 검색 지표가 채점돼 있는데,
기준선 조사에서 "실제로 존재하는 문서를 out-of-domain으로 표기"한 라벨 오류로 판명돼
정답 문서를 붙인 그 문항이다.

주의: Langfuse의 실제 run 이름에는 생성 시각이 접미사로 붙는다
(`... -0913-1942 - 2026-09-13T19:42:51.836626Z`). 이름 안의 `0913-1942`는 UTC이고
KST로는 09-14 04:42이라, 문서의 "2026-09-14 측정"과 같은 실행이다.

## 2026-09-14

### 전 지표 재측정 — 여섯 지표가 모두 온전히 채점된 첫 실행

RRF 전환과 RAGAS `max_tokens` 수정 이후 45문항 전체를 다시 쟀다.
실행: `qa-rrf_k60-top5-cand50-chars3000-recency0.04-temp0-judge_solar-judge-0913-1942`

| 지표 | 값 | 채점 문항 |
|---|---|---|
| `retrieval_hit@5` | 0.974 | 38 / 38 |
| `retrieval_mrr` | 0.908 | 38 / 38 |
...
**`채점 누락 없음`.** 이전까지의 `ragas_faithfulness` 값은 모두 일부 문항이 빠진 평균이었다.
직전 실행(09-13)의 0.887은 40문항 평균이었고, 빠졌던 5문항을 포함하니 0.856으로 내려갔다.
**누락된 평균이 실제보다 높게 나오고 있었다는 뜻이다.**

`ragas_context_precision`이 0.753(09-08, min-max) -> **0.851**로 올랐다.
가져온 문서 중 실제로 답변에 쓰인 비율이며, `hit@5`·`MRR`로는 드러나지 않는 개선이다.
`answer_correctness` 0.970 -> 0.958의 차이는 45문항 중 한 문항이 안 되는 폭이라 노이즈 범위다.

**자체 지표와 표준 지표의 격차가 일정하다.**

| 측정 시점 | 자체 `answer_faithfulness` | 표준 `ragas_faithfulness` | 차이 |
|---|---|---|---|
| 2026-09-01 기준선 | 0.951 | 0.838 | 0.113 |
| 2026-09-14 (누락 0) | 0.973 | 0.856 | **0.117** |

결합 방식과 분석기를 여러 번 바꾸는 동안에도 0.11~0.12로 유지된다.
우연이 아니라 두 구현의 판정 방식 차이(답변 전체를 한 번에 보느냐, 문장 단위로 쪼개느냐)에서
오는 구조적 편향이다.

**남은 실패 1건.** `qa-v2-021`이 `hit=0.0 / faithfulness=1.0 / correctness=0.0`으로
검색 실패로 자동 분류됐다. 질문이 문서를 특정하지 못하는 건이며(같은 날짜 회의 5건)
한 문항에 맞춰 전역 스코어링을 바꾸는 것은 과적합이라 유지한다.
