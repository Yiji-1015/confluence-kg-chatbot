...
- **min-max 정규화의 절벽**: 각 리스트의 꼴찌가 항상 정확히 0.0이라, 후보 풀 마지막
  ...
  **주의 - 결합 방식을 바꾸면 `RECENCY_BOOST_MAX`를 반드시 다시 잡아야 한다.**
  `search_hybrid`는 가중합을 가중치 합(4+6)으로 나누므로 결합 점수 범위는 **0~1**이고,
  가산점 0.04는 그 **4%**에 해당한다. (2026-09-13 정정: 이전에는 범위를 0~10,
  비중을 0.4%로 적었으나 나눗셈을 빠뜨린 계산이었다. recency 측정 자체는 나눗셈이
  있는 코드로 했으므로 "동점 근처에서만 작동한다"는 결론은 그대로 유효하다.)
  RRF(k=60)로 바꾸면 범위가 0~0.16이 되어 같은 0.04가 25%를 차지하고, 사실상
  최신순 정렬이 된다. 검색 지표만 보면 드러나지 않을 수 있다. 최신 문서가 정답인
  문항에서는 오히려 점수가 오르기 때문이다. 전환한다면 recency를 끈 상태로 먼저
  비교해야 한다.

- **`PORTFOLIO_SLIDES.md`의 세 방식 비교 예시가 재현되지 않는다 (2026-09-13 확인).**
  "휴가 신청 절차"에서 BM25 단독 1위를 "어플라이언스 사용용도 구분"(18.91)으로 적었으나,
  현재 코퍼스(3,211청크)에서는 정답인 "3년 근속 주기 Refresh 휴가 제도"(14.93)가 1위다.
  2,979청크 시점 측정값이고 코퍼스가 늘며 IDF가 바뀌었다. 갱신하거나 내려야 한다.
