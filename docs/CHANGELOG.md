...
- **분석기에 `lowercase` 필터가 없다 (2026-09-13 확인).** `nori_analyzer`가 `nori_tokenizer`
  하나뿐이라 영문이 원형 그대로 색인된다. `'BlackBird'`와 `'blackbird'`가 다른 토큰이라
  매칭되지 않고, BM25 1위가 실제로 갈린다(`BlackBird 서버 IP` -> 정답 15.95 /
  `blackbird 서버 IP` -> 다른 문서 14.25). 사내 문서에 `lk-ocr-api`, `HashiCorp` 같은
  영문 고유명사가 많고 사용자가 소문자로 칠 가능성이 높다. `ELASTICSEARCH.md`는 이미
  `lowercase` 필터를 권장하고 있으나 구현이 따르지 않았다.

- **`decompound_mode`가 기본값이다 (2026-09-13 확인).** `'정보처리기사'`가
  `['정보','처리','기사']`로만 색인되고 원형이 남지 않는다. `ELASTICSEARCH.md`의 권장값은
  `mixed`다.

  위 두 항목의 처리 순서:
  1. 새 버전 인덱스를 권장 분석기 설정으로 생성
  2. `_reindex`로 복사 — **기존 `text_vector`를 그대로 옮기므로 임베딩 API 호출이 없다**
  3. `evaluation/compare_fusion.py`로 전후 비교 (v2/v4 양쪽)
  4. 좋으면 별칭 전환, 아니면 그대로 둔다

- **`number_of_replicas`가 매핑에 없다 (2026-09-13 확인).** ES 기본값 1이 적용되어 단일 노드에서
  shard 1개가 영구 unassigned 상태이고 클러스터가 `yellow`다. 운영 신호가 항상 노란색이면
  진짜 문제를 못 알아본다. 이건 동적 설정이라 **재색인 없이** 바꿀 수 있다
  (`PUT <index>/_settings {"number_of_replicas": 0}`). `ELASTICSEARCH.md`의 기준값도 0이다.

- **`ELASTICSEARCH.md`의 "권장 핵심 mapping"이 실제 코드와 다르다.** 필드명(`embedding`/`page_id`/
  `content` vs `text_vector`/`doc_id`/`text`), 분석기 설정, `ancestor_ids`·`metadata` 유무가
  어긋난다. 문서가 먼저 쓰이고(2026-08-19) 구현이 뒤따르며 갈라졌다. 기준 문서를 자처하고
  있으므로 코드에 맞추거나, 코드를 문서에 맞춘 뒤 그 사실을 적어야 한다.
  대조표는 `RETRIEVAL.md`에 있다.
