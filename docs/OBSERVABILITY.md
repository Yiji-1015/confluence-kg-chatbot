...
02 · RAG 단계별 실패(`rag_stage_errors_total`)는 미리 등록할 수 없다. 파이썬 `Counter`도
라벨 조합이 처음 쓰일 때 시계열을 만드는데, 이 지표의 라벨은 `(stage, exception)`이고
`exception`은 예외 클래스 이름이라 값을 예측할 수 없기 때문이다.

패널에 계열을 하나 추가했다.

```promql
absent(rag_stage_errors_total) * 0      # legendFormat: 실패 없음
```

`absent()`는 매칭되는 시계열이 하나도 없을 때만 값을 낸다. 그래서 실패가 없는 동안에는
0 선이 그려지고, 실제 실패가 생기면 그 계열은 사라지고 단계별 계열이 나타난다.
`or vector(0)`과 달리 라벨 없는 유령 계열이 상시로 남지 않는다.

### 계층별 지연 분해를 채팅 경로로 한정

패널 설명은 "1)과 2)의 차이가 backend 자체 시간"이라고 적혀 있는데, 1) `backend 전체`가
actuator만 제외한 **모든 URI**를 담고 있었다. 가벼운 `GET /api/sessions`가 섞이면 p95가
그쪽으로 끌려가 1)이 2)보다 작아지고, 뺄셈이 음수가 된다.

네 계열을 모두 채팅 경로로 한정했다(`uri="/api/chat"`, `uri="/internal/chat"`).
한정 전후로 같은 트래픽에서 이렇게 달라진다.

| 계열 | 한정 전 | 한정 후 |
|---|---|---|
| 1) backend | 0.010s (목록 조회에 끌려감) | 5.583s |
| 2) ai-server 호출 | 2.266s | 5.583s |
| 3) LLM 생성 | 4.850s | 4.850s |
| 4) ES 검색 | 0.048s | 0.048s |

이제 1) − 2) ≈ 0 (backend는 위임만 한다), 2) − (3+4) ≈ 0.7s (ai-server 자체 시간)로
설명대로 읽힌다. API별 지연은 02번 대시보드의 `API별 p95 지연`이 따로 답한다.

대시보드는 `updateIntervalSeconds: 30`으로 파일에서 다시 읽으므로 Grafana 재시작은
필요 없다.
