...
#
# attachments^1.0: 본문과 같은 무게. best_fields라 최고 점수 필드만 채택되므로, 파일명으로
# 묻는 질문에서는 이 필드가 이기고 그 외에는 개입하지 않는다.
#
# 처음에 1.5로 뒀다가 내렸다. 색인 대상을 pdf/excel에서 문서 형식 전반으로 넓히자
# (고유 234종 -> 364종) 1.5가 v4 paraphrase에서 손해로 돌아섰다. 첨부 데이터가 늘수록
# 높은 가중치의 간섭도 같이 커진다.
#
#   가중치        v2 hit@5/MRR      v4 hit@5/MRR
#   1.5          0.974 / 0.895     0.684 / 0.520   <- v4에서 기준선보다 낮음
#   1.0          0.974 / 0.890     0.711 / 0.525   <- 기준선과 동일
#   첨부 제외      0.974 / 0.890     0.711 / 0.525
#
# 차이는 38문항 중 1문항이라 노이즈 범위다. 다만 1.0이 어느 쪽에서도 손해가 아니면서
# 첨부로만 찾히는 문서는 그대로 찾으므로 1.0을 택했다. 첨부 질문이 포함된 평가셋이
# 생기면 그때 다시 정한다.
#
# 상수로 빼둔 이유는 가중치를 바꿔가며 지표를 재기 위함이다 (evaluation/compare_fusion.py).
_BM25_FIELDS = ["title^2.0", "attachments^1.0", "text"]
