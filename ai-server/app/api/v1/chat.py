...
@observe_rag("confluence-rag-chat")
async def process_chat(request: ChatRequest) -> ChatResponse:
    ...
    try:
        ...
        with trace_attributes(
            ...
            with stage("embedding", chars=len(search_query)):
            ...
            with stage("search", query=search_query):
                es_results = await search_hybrid_async(
                    query_text=search_query,
                    query_vector=query_vector,
                )
            record_retrieved(len(es_results or []))
            ...
            # 5. Context 결합 (실서비스와 평가가 같은 포맷을 쓰도록 공용 헬퍼 사용)
            with stage("context_build", documents=len(sources)):
