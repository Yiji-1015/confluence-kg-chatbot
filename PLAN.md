...
동일 모델 계열을 사용하여 embedding space를 유지한다.
...
Python AI Server는 LiteLLM Gateway를 통해 LLM을 호출한다.
...
- Embedding: 동일 vector space를 유지할 수 있는 모델만 fallback
...
- [ ] 사내 BGE-M3 API 접근 가능 여부 확인
- [ ] Local BGE-M3 serving 방식 결정
- [ ] LiteLLM에서 사내 embedding endpoint 연동 방식 검증
